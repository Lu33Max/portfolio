...
const SkillsInfoCard = ({ heading, skills }: InfoCardArgs) => {
  return (
    <div className='skills-info-card min-h-[23rem] rounded-xl border-2 border-solid border-[#6751b9] bg-[rgba(22, 17, 47, 0.4)] backdrop-blur-sm'>
      <h6 className='text-lg max-lg:text-[1.1rem] font-medium px-8 py-3 bg-gradient-to-t from-[#ac96ff] to-[#7c5fe6] text-transparent bg-clip-text border-b-2 border-solid border-b-[#6751b9]'>{heading}</h6>
        ...
        {skills.map((item, i) => (
          <React.Fragment key={`skill_${i}`} >
            <div className='skill-info flex items-center justify-between'>
              <p className='text-base font-medium max-lg:text-[0.9rem]'>{item.skill}</p>
              <p className='percentage text-[#dd8cfa] text-base font-medium max-lg:text-[0.9rem]'>{item.percentage}</p>
            </div>
